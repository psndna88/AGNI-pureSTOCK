/* arch/arm/mach-omap2/board-espresso-cmcc-modems.c
 *
...
static void sprd_on(struct modemctl *mc)
{
	...
	if ((system_rev == 4) && (gpio_get_value(uart_sel) == 0)) {
		gpio_set_value(uart_sel, 1);
		pr_err("[%s] Rev0.0 board uart_sel set high(%d)\n",
			__func__, gpio_get_value(uart_sel));
	}
}
...
static void sprd_boot(struct modemctl *mc)
{
	...
	if ((system_rev  == 4)/*&& (!(init_switch_sel & 0x02))*/) {
		pr_err("[%s] Rev0.0 board uart_sel set low\n",
			__func__);
		gpio_set_value(uart_sel, 0);
	}
}
...
{
	pr_debug("[%s]\n", __func__);
	if (mc->gpio_ap_cp_int2 == -EINVAL) {
		pr_err("[%s] no gpio data\n", __func__);
		return;
	}
