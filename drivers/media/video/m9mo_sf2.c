...
static int m9mo_set_capture_mode(struct v4l2_subdev *sd, int val)
{
	...
	case RUNNING_MODE_COLOUR_BRACKET:
		cam_trace("~~~~~~ Colour Bracket mode ~~~~~~\n");
		/* Like PRO ON */
		err = m9mo_writeb(sd, M9MO_CATEGORY_PRO_MODE,
			0x00, 0x01);
		CHECK_ERR(err);

		/* Colour Bracket Mode ON */
		err = m9mo_writeb(sd, M9MO_CATEGORY_PRO_MODE,
			0x01, 0x11);
		CHECK_ERR(err);
		break;
	...
	} else if(state->running_capture_mode == RUNNING_MODE_LOWLIGHT) {
		int_en |= M9MO_INT_CAPTURE|M9MO_INT_SOUND;
	} else {
		int_en |= M9MO_INT_FRAME_SYNC;
	}
...
static int m9mo_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_CAMERA_CAC_EN:
		err = m9mo_readb(sd, M9MO_CATEGORY_CAPPARM,
				M9MO_CAPPARM_CAC_EN, &val);
		cam_trace("HSW  0 V4L2_CID_CAMERA_CAC_EN val:%d\n", val);
		ctrl->value = val;
		break;
...
static int m9mo_start_capture(struct v4l2_subdev *sd, int frame_num)
{
	...
	} else if (state->running_capture_mode == RUNNING_MODE_COLOUR_BRACKET) {
		cam_trace("running_capture_mode=RUNNING_MODE_COLOUR_BRACKET frame_num:%d\n", frame_num);
		/* Select image number of frame */
		err = m9mo_writeb(sd, M9MO_CATEGORY_CAPCTRL,
				M9MO_CAPCTRL_FRM_SEL, frame_num);
		CHECK_ERR(err);
...
static int m9mo_set_PASM_mode(struct v4l2_subdev *sd, int val)
{
	...
	case MODE_PICTURE_WIZARD_BKT:
		cam_trace("MODE_PICTURE_WIZARD_BKT SETTING!!!!\n");		
		break;
...
static int m9mo_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_CAMERA_AE_POSITION_X:
		...
		cam_trace("HSW V4L2_CID_CAMERA_AE_POSITION_X X:%d\n", state->ae_touch.pos_x);
		err = m9mo_writew(sd, M9MO_CATEGORY_MON,
				M9MO_MON_TOUCH_AE_WINDOW_POSITION_X, state->ae_touch.pos_x);
		CHECK_ERR(err);
		break;

	case V4L2_CID_CAMERA_AE_POSITION_Y:
		state->ae_touch.pos_y = ctrl->value;
		cam_trace("HSW V4L2_CID_CAMERA_AE_POSITION_Y Y:%d\n", state->ae_touch.pos_y);
		err = m9mo_writew(sd, M9MO_CATEGORY_MON,
				M9MO_MON_TOUCH_AE_WINDOW_POSITION_Y, state->ae_touch.pos_y);
		CHECK_ERR(err);
		break;

	case V4L2_CID_CAMERA_AE_WINDOW_SIZE_WIDTH:
		state->ae_touch.window_size_width = ctrl->value;
		cam_trace("HSW V4L2_CID_CAMERA_AE_WINDOW_SIZE_WIDTH X:%d\n", state->ae_touch.window_size_width);
		err = m9mo_writew(sd, M9MO_CATEGORY_MON,
				M9MO_MON_TOUCH_AE_WINDOW_WIDTH, state->ae_touch.window_size_width);
		CHECK_ERR(err);
		break;

	case V4L2_CID_CAMERA_AE_WINDOW_SIZE_HEIGHT:
		state->ae_touch.window_size_height = ctrl->value;
		cam_trace("HSW V4L2_CID_CAMERA_AE_WINDOW_SIZE_HEIGHT Y:%d\n", state->ae_touch.window_size_height);
		err = m9mo_writew(sd, M9MO_CATEGORY_MON,
				M9MO_MON_TOUCH_AE_WINDOW_HEIGHT, state->ae_touch.window_size_width);
		CHECK_ERR(err);
#if 0
		/* Temp Start AE */
		state->ae_touch.status = 1;
			cam_trace("HSW TEMP!!!! V4L2_CID_CAMERA_AE_PREVIEW_TOUCH_CTRL status:%d\n", state->ae_touch.status);
			/* Start Touch AE */
			err = m9mo_writeb(sd, M9MO_CATEGORY_MON,
				M9MO_MON_TOUCH_AE_START, state->ae_touch.status);
			CHECK_ERR(err);
		/* Temp Start AE */	
#endif
		break;
	...
	case V4L2_CID_CAMERA_AE_PREVIEW_TOUCH_CTRL:
		cam_trace("HSW  0 V4L2_CID_CAMERA_AE_PREVIEW_TOUCH_CTRL status:%d\n", state->ae_touch.status);
		if(state->ae_touch.status == ctrl->value) {
			cam_trace("HSW  1 V4L2_CID_CAMERA_AE_PREVIEW_TOUCH_CTRL status:%d\n", state->ae_touch.status);
		}
		else {
			state->ae_touch.status = ctrl->value;
			cam_trace("HSW 2 V4L2_CID_CAMERA_AE_PREVIEW_TOUCH_CTRL status:%d\n", state->ae_touch.status);
			/* Start Touch AE */
			err = m9mo_writeb(sd, M9MO_CATEGORY_MON,
				M9MO_MON_TOUCH_AE_START, state->ae_touch.status);
			CHECK_ERR(err);
	...
	case V4L2_CID_CAMERA_CAC_EN:
	{
		int cac_en = 0;
		err = m9mo_readb(sd, M9MO_CATEGORY_CAPPARM,
				M9MO_CAPPARM_CAC_EN, &cac_en);
		cam_trace("HSW  0 V4L2_CID_CAMERA_CAC_EN ctrl->value:%d, cac_en:%d\n", ctrl->value, cac_en);
		if(cac_en == ctrl->value) {
			cam_trace("HSW  1 V4L2_CID_CAMERA_CAC_EN cac_en:%d\n", cac_en);
		}
		else {
			cam_trace("HSW 2 V4L2_CID_CAMERA_CAC_EN ctrl->value:%d\n", ctrl->value);
			err = m9mo_writeb(sd, M9MO_CATEGORY_CAPPARM,
				M9MO_CAPPARM_CAC_EN, ctrl->value);
			CHECK_ERR(err);
		}
		break;
	}
...
re_init:
	...
	err = m9mo_check_fw(sd);
#if defined(CONFIG_MACH_GC2PD) || defined(CONFIG_MACH_GC1)
