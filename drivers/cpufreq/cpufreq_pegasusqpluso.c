/*
 *  drivers/cpufreq/cpufreq_pegasusqpluso.c
 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_PEGASUSQPLUSO
static
#endif
struct cpufreq_governor cpufreq_gov_pegasusqpluso = {
	.name                   = "pegasusqpluso",
...
int cpufreq_pegasusqpluso_cpu_lock(int num_core)
{
	int prev_lock;
...
int cpufreq_pegasusqpluso_cpu_unlock(int num_core)
{
...
void cpufreq_pegasusqpluso_min_cpu_lock(unsigned int num_core)
{
	int online, flag;
	struct cpu_dbs_info_s *dbs_info;
...
void cpufreq_pegasusqpluso_min_cpu_unlock(void)
{
...
/* cpufreq_pegasusqpluso Governor Tunables */
				  ...
				  const char *buf, size_t count)
{
	...
	if (ret != 1)
		return -EINVAL;
	if (input == 0)
		cpufreq_pegasusqpluso_min_cpu_unlock();
	else
		cpufreq_pegasusqpluso_min_cpu_lock(input);
	return count;
}
				  ...
				  const char *buf, size_t count)
{
	...
	if (prev_lock)
		cpufreq_pegasusqpluso_cpu_unlock(prev_lock);
	...
	ret = cpufreq_pegasusqpluso_cpu_lock(input);
	if (ret) {
...
#endif
static void cpufreq_pegasusqpluso_early_suspend(struct early_suspend *h)
{
...
static void cpufreq_pegasusqpluso_late_resume(struct early_suspend *h)
{
	struct cpu_dbs_info_s *dbs_info;
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	dvfs_workqueuepluso = create_workqueue("kpegasusqpluso");
	...
	ret = cpufreq_register_governor(&cpufreq_gov_pegasusqpluso);
	if (ret)
		goto err_reg;

#ifdef CONFIG_HAS_EARLYSUSPEND
	early_suspend.level = EARLY_SUSPEND_LEVEL_DISABLE_FB;
	early_suspend.suspend = cpufreq_pegasusqpluso_early_suspend;
	early_suspend.resume = cpufreq_pegasusqpluso_late_resume;
#endif
...
{
	cpufreq_unregister_governor(&cpufreq_gov_pegasusqpluso);
	destroy_workqueue(dvfs_workqueuepluso);
	kfree(hotplug_historypluso);
}

MODULE_AUTHOR("ByungChang Cha <bc.cha@samsung.com>");
MODULE_DESCRIPTION("'cpufreq_pegasusqpluso' - A dynamic cpufreq/cpuhotplug governor");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_PEGASUSQPLUSO
